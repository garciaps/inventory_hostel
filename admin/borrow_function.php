...
switch ($operation) {
    case 'insert':
        ...
        $checkSql = "SELECT COUNT(*) FROM borrower WHERE name = :name";
        ...
        if ($count > 0) {
            // Display a non-blocking modal overlay with JavaScript
            echo '<script>
                    var confirmation = confirm("The name already exists!");
                    if (confirmation) {
